...
# Eval-only AIME benchmark for Qwen2.5 ReTool checkpoints.
#
# Select a model with:
#   BENCH_MODEL=qwen2.5-3b|qwen2.5-7b|qwen2.5-3b-sft|qwen2.5-7b-sft
#
...
stop_existing_runtime() {
   ...
   pkill -9 -f "ray::|raylet|gcs_server|dashboard|runtime_env_agent|log_monitor" 2>/dev/null || true
   pkill -9 -f "${SLIME_ROOT:-/root/slime}.*train.py" 2>/dev/null || true
}
...
BENCH_MODEL="$(normalize_model_name "${BENCH_MODEL:-qwen2.5-3b}")"
...
BENCH_EVAL_MAX_CONTEXT_LEN=${BENCH_EVAL_MAX_CONTEXT_LEN:-65536}
BENCH_EVAL_MAX_RESPONSE_LEN=${BENCH_EVAL_MAX_RESPONSE_LEN:-49152}
...
BENCH_ROLLOUT_MAX_CONTEXT_LEN=${BENCH_ROLLOUT_MAX_CONTEXT_LEN:-${BENCH_EVAL_MAX_CONTEXT_LEN}}
BENCH_ROLLOUT_MAX_RESPONSE_LEN=${BENCH_ROLLOUT_MAX_RESPONSE_LEN:-${BENCH_EVAL_MAX_RESPONSE_LEN}}
BENCH_SGLANG_CONTEXT_LENGTH=${BENCH_SGLANG_CONTEXT_LENGTH:-${BENCH_EVAL_MAX_CONTEXT_LEN}}
BENCH_RETOOL_MAX_NEW_TOKENS_PER_TURN=${BENCH_RETOOL_MAX_NEW_TOKENS_PER_TURN:-0}
...
case "${BENCH_MODEL}" in
   qwen2.5-3b | 3b)
      BENCH_MODEL_TAG=qwen2.5-3B
      BENCH_HF_CHECKPOINT=${BENCH_HF_CHECKPOINT:-${QWEN2_5_3B_HF}}
      BENCH_LOAD=${BENCH_LOAD:-${QWEN2_5_3B_REF_LOAD}}
      BENCH_REF_LOAD=${BENCH_REF_LOAD:-${QWEN2_5_3B_REF_LOAD}}
      BENCH_ROTARY_BASE=${BENCH_ROTARY_BASE:-${QWEN2_5_3B_ROTARY_BASE}}
      ...
      ;;
   qwen2.5-7b | 7b)
      BENCH_MODEL_TAG=qwen2.5-7B
      BENCH_HF_CHECKPOINT=${BENCH_HF_CHECKPOINT:-${QWEN2_5_7B_HF}}
      BENCH_LOAD=${BENCH_LOAD:-${QWEN2_5_7B_REF_LOAD}}
      BENCH_REF_LOAD=${BENCH_REF_LOAD:-${QWEN2_5_7B_REF_LOAD}}
      BENCH_ROTARY_BASE=${BENCH_ROTARY_BASE:-${QWEN2_5_7B_ROTARY_BASE}}
      ...
      ;;
   qwen2.5-3b-sft | 3b-sft)
      BENCH_MODEL_TAG=qwen2.5-3B-retool-sft
      BENCH_HF_CHECKPOINT=${BENCH_HF_CHECKPOINT:-${QWEN2_5_3B_HF}}
      DEFAULT_LOAD="$(first_existing_path "${QWEN2_5_3B_SFT_SAVE}" "${QWEN2_5_3B_SFT_CLEAN_DIST}" "${QWEN2_5_3B_SFT_SAVE}")"
      ...
      source "${SLIME_ROOT}/scripts/models/qwen2.5-3B.sh"
      ;;
   qwen2.5-7b-sft | 7b-sft)
      BENCH_MODEL_TAG=qwen2.5-7B-retool-sft
      BENCH_HF_CHECKPOINT=${BENCH_HF_CHECKPOINT:-${QWEN2_5_7B_HF}}
      DEFAULT_LOAD="$(first_existing_path "${QWEN2_5_7B_SFT_CLEAN_DIST}" "${QWEN2_5_7B_SFT_CLEAN_DIST}" "${QWEN2_5_7B_SFT_SAVE}")"
      BENCH_LOAD=${BENCH_LOAD:-${DEFAULT_LOAD}}
      BENCH_REF_LOAD=${BENCH_REF_LOAD:-${BENCH_LOAD}}
      BENCH_ROTARY_BASE=${BENCH_ROTARY_BASE:-${QWEN2_5_7B_ROTARY_BASE}}
      BENCH_TENSOR_MODEL_PARALLEL_SIZE=${BENCH_TENSOR_MODEL_PARALLEL_SIZE:-2}
   ...
   *)
      ...
      echo "Expected qwen2.5-3b, qwen2.5-7b, qwen2.5-3b-sft, or qwen2.5-7b-sft." >&2
      exit 1
      ;;
esac
...
OPTIMIZER_ARGS=(
   ...
   --lr-decay-iters "${BENCH_LR_DECAY_ITERS:-1}"
...
if [ "${BENCH_USE_WANDB}" = "1" ]; then
   DEFAULT_BENCH_WANDB_GROUP="${BENCH_MODEL_TAG}-aime-pass${BENCH_EVAL_N_SAMPLES_PER_PROMPT}-ctx${BENCH_EVAL_MAX_CONTEXT_LEN}"
   WANDB_ARGS=(
      --use-wandb
      --wandb-project "${BENCH_WANDB_PROJECT}"
      --wandb-group "${BENCH_WANDB_GROUP:-${DEFAULT_BENCH_WANDB_GROUP}}"
...
SGLANG_ARGS=(
   ...
   --sglang-context-length "${BENCH_SGLANG_CONTEXT_LENGTH}"
)
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    ...
    \"RETOOL_MAX_NEW_TOKENS_PER_TURN\": \"${BENCH_RETOOL_MAX_NEW_TOKENS_PER_TURN}\",
...
echo "Eval samples/context/response: ${BENCH_EVAL_N_SAMPLES_PER_PROMPT}/${BENCH_EVAL_MAX_CONTEXT_LEN}/${BENCH_EVAL_MAX_RESPONSE_LEN}"
echo "SGLang context length: ${BENCH_SGLANG_CONTEXT_LENGTH}"
echo "ReTool max new tokens per turn: ${BENCH_RETOOL_MAX_NEW_TOKENS_PER_TURN}"
