...
# Qwen2.5-3B-Instruct ReTool SFT before OPD.
...
CKPT_ARGS=(
   ...
   --ref-load "${QWEN2_5_3B_REF_LOAD}"
   --save "${QWEN2_5_3B_SFT_SAVE}"
   --save-interval "${SFT_SAVE_INTERVAL}"
   --rotary-base "${QWEN2_5_3B_ROTARY_BASE}"
)

if [ -n "${QWEN2_5_3B_SFT_LOAD}" ]; then
   CKPT_ARGS+=(--load "${QWEN2_5_3B_SFT_LOAD}")
fi

if [ -n "${QWEN2_5_3B_SFT_SAVE_HF}" ]; then
   CKPT_ARGS+=(--save-hf "${QWEN2_5_3B_SFT_SAVE_HF}")
fi
...
if [ "${USE_WANDB:-0}" = "1" ]; then
   WANDB_ARGS=(
      ...
      --wandb-group "${WANDB_GROUP:-qwen2.5-3B-retool-sft}"
   )
   if [ -n "${WANDB_KEY:-}" ]; then
