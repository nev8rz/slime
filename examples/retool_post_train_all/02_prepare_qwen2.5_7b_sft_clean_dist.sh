...
# Export the Qwen3-8B ReTool SFT checkpoint to HF, then convert that HF
...
SFT_ITER_DIR="$(latest_megatron_ckpt_dir "${QWEN3_8B_SFT_SAVE}")"
if [ ! -d "${SFT_ITER_DIR}" ]; then
   echo "SFT checkpoint directory does not exist: ${SFT_ITER_DIR}" >&2
   exit 1
fi

prepare_output_dir "${QWEN3_8B_SFT_HF_EXPORT}"
prepare_output_dir "${QWEN3_8B_SFT_CLEAN_DIST}"

cd "${SLIME_ROOT}"

PYTHONPATH="${MEGATRON_PATH}:${PYTHONPATH:-}" python3 tools/convert_torch_dist_to_hf.py \
   --input-dir "${SFT_ITER_DIR}" \
   --output-dir "${QWEN3_8B_SFT_HF_EXPORT}" \
   --origin-hf-dir "${QWEN3_8B_HF}" \
   --vocab-size 151936
...
PYTHONPATH="${MEGATRON_PATH}:${PYTHONPATH:-}" torchrun \
   ...
   "${MODEL_ARGS[@]}" \
   --hf-checkpoint "${QWEN3_8B_SFT_HF_EXPORT}" \
   --rotary-base "${QWEN3_8B_ROTARY_BASE}" \
   --save "${QWEN3_8B_SFT_CLEAN_DIST}"
