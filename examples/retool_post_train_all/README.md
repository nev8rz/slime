...
5. Serve the trained Qwen2.5-7B-Instruct HF checkpoint as an SGLang teacher and run OPD into the 4B student.

## Files

- `00_prepare_qwen2.5_checkpoints.sh`: optional data preparation and HF to torch-dist conversion for the 8B teacher and 4B student.
...
- `04_qwen2.5_3b_opd_from_qwen2.5_7b_retool.sh`: Qwen2.5-3B-Instruct OPD using the trained 8B ReTool teacher.
- `05_qwen2.5_retool_aime_bench.sh`: eval-only AIME benchmark. Select checkpoints with `BENCH_MODEL`.
- `06_qwen2.5_retool_aime_bench_all.sh`: runs the AIME benchmark for 4B, 8B, 4B SFT, and 8B SFT.
- `run_all.sh`: runs the stages in order.
- `generate_with_retool.py` and `tool_sandbox.py`: copied ReTool rollout helpers used by RL and OPD. The custom generate path adds ReTool stop strings (`</tool_call>` and Qwen chat end tokens) before each SGLang request so a completed tool call or final answer does not keep sampling into hallucinated text.
...
hf download --repo-type dataset zhuzilin/aime-2025 --local-dir "${DATA_ROOT}/aime-2025"
```
...
Benchmark W&B defaults to `BENCH_WANDB_PROJECT=retool-bench`, so it is separated from training runs. The benchmark script relies on `WANDB_API_KEY` from the environment instead of passing it as a CLI argument, keeping Ray entrypoint logs clean. With `BENCH_EVAL_N_SAMPLES_PER_PROMPT=16` and `BENCH_LOG_PASSRATE=1`, the key `eval/<dataset>-pass@16` is all16, meaning all 16 samples for a problem are correct. The key `eval/<dataset>-any@16` is any16, meaning at least one of the 16 samples is correct. The key `eval/<dataset>-mean@16` is the average correct fraction across the 16 samples for each problem, averaged over problems.
...
| --- | --- | --- |
| `LOG_PASSRATE` | `1` | Adds `--log-passrate`, which records train rollout `passrate/pass@k` as any-correct@k. Eval records `eval/<dataset>-pass@k` as all-correct@k, `eval/<dataset>-any@k` as any-correct@k, and `eval/<dataset>-mean@k` as mean-correct@k. With `EVAL_N_SAMPLES_PER_PROMPT=8`, eval logs k=1/2/4/8. |
...
| `EVAL_MAX_CONTEXT_LEN` | `40960` | Total eval context budget used by the ReTool custom generate function. |
| `EVAL_MAX_RESPONSE_LEN` | `32768` | Eval generation response token cap. |
...
| `RL_CONTEXT_PARALLEL_SIZE` | `2` | Context parallel size for 8B RL training. This keeps two data-parallel replicas on an 8 GPU node with `TP=2`. |
| `RL_MAX_TOKENS_PER_GPU` | `16384` | Dynamic training token budget per GPU for 8B RL. |
| `RL_ROLLOUT_MAX_CONTEXT_LEN` | `32768` | Total 8B RL rollout context budget. |
| `RL_ROLLOUT_MAX_RESPONSE_LEN` | `24576` | 8B RL generation response token cap. |
...
| `OPD_CONTEXT_PARALLEL_SIZE` | `2` | Context parallel size for 4B OPD training. |
| `OPD_MAX_TOKENS_PER_GPU` | `16384` | Dynamic training token budget per GPU for 4B OPD. |
| `OPD_ROLLOUT_MAX_CONTEXT_LEN` | `32768` | Total 4B OPD rollout context budget. |
| `OPD_ROLLOUT_MAX_RESPONSE_LEN` | `24576` | 4B OPD generation response token cap. |
...
The ReTool SFT converter folds each assistant turn's reasoning into the visible `content`. Text before a tool call becomes the assistant `content` that precedes the `tool_calls`; for the final answer turn, text before `<answer>...</answer>` is prepended to the normalized `Answer: \boxed{...}` content. Nothing is stored in `reasoning_content`. The SFT scripts still pass `--apply-chat-template-kwargs '{"enable_thinking": false}'`, which Qwen2.5 simply ignores, and use `--loss-mask-type qwen` for the Qwen2.5 chat template.

The local SFT conversion rewrites the original ReTool transcript into real multi-turn tool-call data. Each sample contains `messages` plus a `tools` column. The source `<code>```python ...```</code>` blocks become assistant `tool_calls` to `code_interpreter`; the following `<interpreter>...</interpreter>` blocks become `tool` role messages; the final `<answer>\boxed{...}</answer>` becomes `Answer: \boxed{...}` visible text. The SFT scripts pass `--tool-key tools`, so Qwen2.5's native chat template injects the `# Tools` system section and renders `<tool_call>...</tool_call>` instead of training on legacy `<code>` tags.

RL and OPD also call the tokenizer's native `apply_chat_template` and pass the code-interpreter tool schema through the `tools` argument. The custom generate path does not prefill any `<think>` block, so the model produces plain reasoning followed by `<tool_call>...</tool_call>` or a final `Answer: \boxed{...}`. The RL/OPD scripts default `RETOOL_THINKING_MODE=no_think`; this knob is now inert for the rollout code (kept only for backward compatibility) because Qwen2.5 has no thinking mode.
...
The 4B OPD stage is pure on-policy distillation in the current script. The rollout reward returned by `slime.rollout.on_policy_distillation.post_process_rewards` is zero, so the useful training signal comes from the teacher KL term that is applied to the token-level advantages.
